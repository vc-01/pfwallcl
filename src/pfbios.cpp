...
char const * const
	PFBios::msg_anim_willstop =
			...
			"\0"
	//   | | TEXT.TEXT.TEXT.TEXT.TEXT.TEXT.TEXT | |
			"Avail. only in NORMAL mode."
			"\0";
char const * const
	PFBios::msg_poweroff_delay_m =
			"Power Off Delayed"
			"\0"
	//   | | TEXT.TEXT.TEXT.TEXT.TEXT.TEXT.TEXT | |
			"Will power off in 4 minutes."
			"\0";
char const * const
...
char * const
	...
	//   | | TEXT.TEXT.TEXT.TEXT.TEXT.TEXT.TEXT | |
			...
			"\0";
char const * const
	PFBios::msg_err_bioscompat =
			"Incompatible BIOS version.$";
char const * const
	PFBios::msg_err_compat =
			"Incompatible system.$";
...
{
	union REGS regs;
	regs.h.ah = 07;					// reset RTC alarm (bye Dosbox)
	...
	unsigned int second)
{
	...
	regs.h.dh = dec2bcd(second); 	// seconds (BCD)
	regs.h.ah = 06;					// set RTC alarm (bye Dosbox)
	int86 (0x1a, &regs, &regs);		// BIOS Timer/Clock Service
}
...
#else // #ifdef NTVDM
	cout << msg << ": "; // print message title
    while (*msg++ != NULL); // move to next string
